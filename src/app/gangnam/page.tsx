...
const POLICY_X     = '25-10'   // matches YY-MM axis format

const TIERS = [
  {
    label:    '25억원 초과',
    cap:      '대출 한도 2억원 (~USD 150k)',
    note:     '강남 평균 거래가의 대부분 해당',
    color:    'text-rose-400',
...
export default function GangnamPage() {
  const [data, setData] = useState<GangnamData | null>(null)

  useEffect(() => {
    fetch('/api/gangnam').then(r => r.json()).then(setData)
  }, [])
  ...
  const volumeChange = data.prePolicy.avgMonthly && data.postPolicy.avgMonthly
    ? ((data.postPolicy.avgMonthly - data.prePolicy.avgMonthly) / data.prePolicy.avgMonthly * 100)
    : null

  const priceChange = data.prePolicy.avgPrice && data.postPolicy.avgPrice
    ? ((data.postPolicy.avgPrice - data.prePolicy.avgPrice) / data.prePolicy.avgPrice * 100)
    : null

  // Chart data: format month to YY-MM
  const volumeChart = data.trends.map(t => ({
    month:  t.month.slice(2),
    거래량: t.transactionCount,
  }))

  const priceChart = data.trends.map(t => ({
    month:       t.month.slice(2),
    평균거래가:  t.avgPrice,
    'm²당가격': t.avgPricePerM2,
  }))
  ...
  // Latest price tier split (most recent month)
  ...
  const policyMonthInData = data.trends.some(t => t.month >= '2025-10')
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex flex-wrap items-center gap-3 mb-1">
          <h1 className="text-xl font-bold text-slate-100">강남구 심층 분석</h1>
          {data.isMock
            ? <MockBadge detail="모의 데이터 — 실제 정책 충격이 반영되지 않습니다. 자정 KST 이후 실제 데이터로 갱신됩니다." />
            : <RealBadge source="국토교통부" />}
        </div>
        <p className="text-slate-400 text-sm">
          2025년 10월 16일 시행된 고가주택 주담대 상한제(10·15 대책)가 서울 최고가 지역인 강남구의
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
        <div className="mb-3">
          ...
          <p className="text-sm font-semibold text-slate-200">
            주택 가격에 따른 3단계 주담대 상한 — 강남 아파트 대부분이 최저 한도 적용
          </p>
        </div>
        ...
        <p className="text-[10px] text-slate-600 mt-3">
          출처: 금융위원회 10·15 주택시장 안정화 대책 보도자료 · 투기과열지구 전 서울 지정 포함
        </p>
      </div>
      ...
      {data.isMock && (
        <div className="bg-orange-950/20 border border-orange-800/30 rounded-xl p-4">
          ...
          <p className="text-xs text-slate-400 leading-relaxed">
            현재 표시 중인 데이터는 MOLIT API 할당량 소진으로 생성된 모의 데이터입니다.
            모의 데이터는 <strong className="text-slate-300">계절성·주기 패턴</strong>은 반영하지만,
            10·15 대출규제에 따른 <strong className="text-slate-300">실제 거래량 급감 효과는 반영하지 않습니다.</strong>
            실제 데이터에서는 2025년 10월 이후 거래량이 뚜렷이 줄고,
            25억 초과 구간 거래 비중이 감소하는 패턴이 나타납니다.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-4">
          ...
          <p className={clsx('text-2xl font-bold mt-1',
            ...
            {data.postPolicy.avgMonthly > 0 ? `${data.postPolicy.avgMonthly}건` : '—'}
          </p>
          <p className="text-xs text-slate-500 mt-1">
            {volumeChange !== null
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={volumeChart} margin={{ top: 16, right: 4, left: 0, bottom: 5 }}>
              ...
              <Tooltip {...tooltipStyle()} formatter={(v: number) => [`${v}건`, '거래량']} />
              <ReferenceLine
                x={POLICY_X}
                stroke="#f59e0b"
                strokeDasharray="4 2"
            ...
            <LineChart data={priceChart} margin={{ top: 16, right: 4, left: 0, bottom: 5 }}>
              ...
              <Tooltip
                ...
                formatter={(v: number, name: string) => [
                  name === '평균거래가' ? `${fmt(v)}원` : fmtPricePerM2(v),
                  name,
                ]}
              />
        ...
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={tierChart} margin={{ top: 16, right: 4, left: 0, bottom: 5 }}>
            ...
            <ReferenceLine
              ...
              label={<RefLabel value={POLICY_LABEL} />}
            />
            <Bar dataKey="15억미만"  stackId="tier" fill="#3b82f6" radius={[0, 0, 0, 0]} />
            <Bar dataKey="15–25억"  stackId="tier" fill="#f59e0b" radius={[0, 0, 0, 0]} />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-700">
                  ...
                  <th className="py-2 pr-6 text-right text-slate-400 font-semibold">
                    규제 후 ({data.postPolicy.months}개월 평균)
                  </th>
              ...
              <tbody className="divide-y divide-slate-700/40">
                <tr>
                  ...
                  <td className="py-3 text-right">
                    {volumeChange !== null ? (
                      <span className={volumeChange < 0 ? 'text-rose-400 font-semibold' : 'text-emerald-400'}>
